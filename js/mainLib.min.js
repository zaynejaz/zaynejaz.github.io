...
for(var a=0,r=e.$item;null!==r&&r!==document&&0===a;){var l=e.css(r,"-webkit-transform")||e.css(r,"-moz-transform")||e.css(r,"transform");l&&"none"!==l&&(a=1,e.image.position="absolute"),r=r.parentNode}
...
i.r(e);var s=function(){function t(){!function(t,e){if(!(t instanceof e))throw new TypeError("Cannot call a class as a function")}(this,t),this.positions={top:0,bottom:0,height:0}}
...
self.timeout=setTimeout(function(){if(curStrPos===curString.length){self.options.onStringTyped(self.arrayPos);if(self.arrayPos===self.strings.length-1){self.options.callback();self.curLoop++;if(self.loop===!1||self.curLoop===self.loopCount)return}
self.timeout=setTimeout(function(){self.backspace(curString,curStrPos)},self.backDelay)}else{if(curStrPos===0)self.options.preStringTyped(self.arrayPos);var nextString=curString.substr(0,curStrPos+1);if(self.attr){self.el.attr(self.attr,nextString)}else{if(self.isInput){self.el.val(nextString)}else if(self.contentType==="html"){self.el.html(nextString)}else{self.el.text(nextString)}}
...
if(curStrPos>self.stopNum){curStrPos--;self.backspace(curString,curStrPos)}else if(curStrPos<=self.stopNum){self.arrayPos++;if(self.arrayPos===self.strings.length){self.arrayPos=0;if(self.shuffle)self.sequence=self.shuffleArray(self.sequence);self.init()}else self.typewrite(self.strings[self.sequence[self.arrayPos]],curStrPos)}},humanize)},shuffleArray:function(array){var tmp,current,top=array.length;if(top)
while(--top){current=Math.floor(Math.random()*(top+1));tmp=array[current];array[current]=array[top];array[top]=tmp}
return array},reset:function(){var self=this;clearInterval(self.timeout);var id=this.el.attr("id");this.el.after('<span id="'+id+'"/>');this.el.remove();if(typeof this.cursor!=="undefined"){this.cursor.remove()}
...
self.fire("ready",e);if(self.options.loop&&!self.options.endTime){var secondsOffset=.1;self.options.endTime=self.player.getDuration()-secondsOffset}
...
swiper.velocity*=params.freeModeMomentumVelocityRatio;data.velocities.length=0;var momentumDuration=1e3*params.freeModeMomentumRatio;var momentumDistance=swiper.velocity*momentumDuration;var newPosition=swiper.translate+momentumDistance;if(rtl){newPosition=-newPosition}
...
if(params.hide){clearTimeout(swiper.scrollbar.timeout);$el[0].style.opacity=1;swiper.scrollbar.timeout=setTimeout(function(){$el[0].style.opacity=0;$el.transition(400)},1e3)}},setTransition:function setTransition(duration){var swiper=this;if(!swiper.params.scrollbar.el||!swiper.scrollbar.el){return}
...
swiper.scrollbar.isTouched=!1;if(params.hide){clearTimeout(swiper.scrollbar.dragTimeout);swiper.scrollbar.dragTimeout=Utils.nextTick(function(){$el.css("opacity",0);$el.transition(400)},1e3)}
...
e.preventDefault();e.stopPropagation();image.isMoved=!0;image.currentX=image.touchesCurrent.x-image.touchesStart.x+image.startX;image.currentY=image.touchesCurrent.y-image.touchesStart.y+image.startY;if(image.currentX<image.minX){image.currentX=image.minX+1-Math.pow(image.minX-image.currentX+1,.8)}
if(image.currentX>image.maxX){image.currentX=image.maxX-1+Math.pow(image.currentX-image.maxX+1,.8)}
if(image.currentY<image.minY){image.currentY=image.minY+1-Math.pow(image.minY-image.currentY+1,.8)}
...
var zFactor=Browser.isSafari||Browser.isUiWebView?-swiperSize/2:0;$wrapperEl.transform("translate3d(0px,0,"+zFactor+"px) rotateX("+(swiper.isHorizontal()?0:wrapperRotate)+"deg) rotateY("+(swiper.isHorizontal()?-wrapperRotate:0)+"deg)")},setTransition:function setTransition(duration){var swiper=this;var $el=swiper.$el;var slides=swiper.slides;slides.transition(duration).find(".swiper-slide-shadow-top, .swiper-slide-shadow-right, .swiper-slide-shadow-bottom, .swiper-slide-shadow-left").transition(duration);if(swiper.params.cubeEffect.shadow&&!swiper.isHorizontal()){$el.find(".swiper-cube-shadow").transition(duration)}}};var EffectCube={name:"effect-cube",params:{cubeEffect:{slideShadows:!0,shadow:!0,shadowOffset:20,shadowScale:.94}},create:function create(){var swiper=this;Utils.extend(swiper,{cubeEffect:{setTranslate:Cube.setTranslate.bind(swiper),setTransition:Cube.setTransition.bind(swiper)}})},on:{beforeInit:function beforeInit(){var swiper=this;if(swiper.params.effect!=="cube"){return}
...
element.style.overflow="hidden";var prefixes=["","-moz-","-o-animation-","-webkit-","-ms-"];var options={animate:!0,patternWidth:100,patternHeight:100,grainOpacity:.1,grainDensity:1,grainWidth:1,grainHeight:1,grainChaos:.5,grainSpeed:20};Object.keys(opt).forEach(function(key){options[key]=opt[key]});var generateNoise=function(){var canvas=doc.createElement("canvas");var ctx=canvas.getContext("2d");canvas.width=options.patternWidth;canvas.height=options.patternHeight;for(var w=0;w<options.patternWidth;w+=options.grainDensity){for(var h=0;h<options.patternHeight;h+=options.grainDensity){var rgb=Math.random()*256|0;ctx.fillStyle="rgba("+[rgb,rgb,rgb,options.grainOpacity].join()+")";ctx.fillRect(w,h,options.grainWidth,options.grainHeight)}}
...
window.grained=grained})(window,document);function initMap(){var e={zoom:16,mapTypeControl:!1,disableDefaultUI:!0,zoomControl:!1,scrollwheel:!1,styles:[{stylers:[{hue:"#ff1a00"},{invert_lightness:!0},{saturation:-100},{lightness:33},{gamma:.5}]},{featureType:"water",elementType:"geometry",stylers:[{color:"#2D333C"}]},{elementType:"labels",stylers:[{visibility:"off"}]},{elementType:"labels.icon",stylers:[{visibility:"off"}]},{featureType:"administrative.land_parcel",stylers:[{visibility:"off"}]},{featureType:"administrative.neighborhood",stylers:[{visibility:"off"}]}]}}!function(n){"use strict";window.onpageshow=function(e){e.persisted&&window.location.reload()},n(window).on("load",function(){var e=n(".preloader");e.find(".spinner").fadeOut(function(){e.fadeOut()}),n(".lines").addClass("finish"),setTimeout(function(){n(".lines").addClass("ready")},2e3),n(".typed-subtitle").length&&1<n(".h-subtitle p").length&&n(".typed-subtitle").each(function(){n(this).typed({stringsElement:n(this).prev(".typing-subtitle"),loop:!0})}),setTimeout(function(){n(".h-subtitles").addClass("ready"),n(".typed-bread").length&&n(".typed-bread").typed({stringsElement:n(".typing-bread"),showCursor:!1})},1e3);var a=location.hash,t=n(a);if(0==a.indexOf("#section-")&&t.length&&n("body, html").animate({scrollTop:n(a).offset().top-68},400),n(".jarallax").length&&n(".jarallax").jarallax(),n(".started-carousel").length){var i=new Swiper(".started-carousel .swiper-container",{init:!1,loop:!1,spaceBetween:0,effect:"fade",slidesPerView:1,simulateTouch:!1,autoplay:{delay:6e3,disableOnInteraction:!1,waitForTransition:!1},navigation:{nextEl:".started .swiper-button-next",prevEl:".started .swiper-button-prev"}});i.on("slideChange",function(){var t=i.realIndex,s=i.slides.length;n(".started-carousel .swiper-slide").removeClass("first"),n(".started-carousel .swiper-slide").each(function(e,a){e<=t-1?n(a).addClass("swiper-clip-active"):n(a).removeClass("swiper-clip-active")}),n(".started-carousel .swiper-slide").each(function(e,a){n(a).css({"z-index":s-e})})}),i.init()}
if(n(".reviews-carousel").length)new Swiper(".reviews-carousel .swiper-container",{loop:!0,spaceBetween:70,slidesPerView:2,autoplay:{delay:6e3},navigation:{nextEl:".reviews-carousel .swiper-button-next",prevEl:".reviews-carousel .swiper-button-prev"},breakpoints:{720:{slidesPerView:1,spaceBetween:40},1200:{slidesPerView:2,spaceBetween:70}}});setTimeout(function(){n(window).scrollTop(n(window).scrollTop()+1)},100)});var e=n(window).width(),a=n(window).height();if(n(".section.started").css({height:a}),n(".logged-in .section.started").css({height:a-32}),e<783&&(n(".section.started").css({height:a}),n(".logged-in .section.started").css({height:a-46})),!n(".grained-off").length){grained("#grained_container",{animate:!0,patternWidth:400,patternHeight:400,grainOpacity:.15,grainDensity:3,grainWidth:1,grainHeight:1})}
1199<e&&(n(document).on("mousemove",function(e){var a=e.pageX,t=e.pageY;n(".cursor-follower").css("transform","translate3d("+a+"px,"+t+"px,0px)")}),n("a, .btn-group").on({mouseenter:function(e){n(".cursor-follower").stop().animate({width:86,height:86,opacity:.1,margin:"-43px 0 0 -43px"},500)},mouseleave:function(e){n(".cursor-follower").stop().animate({width:26,height:26,opacity:.4,margin:"-13px 0 0 -13px"},500)}})),n(".hover-masks a").each(function(){var e=n(this).html();n(this).wrapInner('<span class="mask-lnk"></span>'),n(this).append('<span class="mask-lnk mask-lnk-hover">'+e+"</span>")}),n(".hover-animated .circle").on({mouseenter:function(e){0===n(this).find(".dotScale").length&&n(this).prepend("<span class='dotScale'></span>");var a=n(this).find(".dotScale");if(a.removeClass("animate"),!a.height()&&!a.width()){var t=Math.max(n(this).outerWidth(),n(this).outerHeight());a.css({height:t,width:t})}
var s=e.pageX-n(this).offset().left-a.width()/2,i=e.pageY-n(this).offset().top-a.height()/2;a.css({top:i+"px",left:s+"px"}).addClass("dotScale-animate"),n(".cursor-follower").addClass("hide")},mouseleave:function(e){var a=n(this).find(".dotScale"),t=e.pageX-n(this).offset().left-a.width()/2,s=e.pageY-n(this).offset().top-a.height()/2;a.css({top:s+"px",left:t+"px"}).removeClass("dotScale-animate"),n(".cursor-follower").removeClass("hide")}}),n("header .top-menu, .typed-bread, .popup-box .bts, .animate-to-page").on("click","a",function(){var e=n(this).attr("href");return 0==e.indexOf("#section-")?(n("body").hasClass("home")||(location.href="/"+e),n("body, html").animate({scrollTop:n(e).offset().top-68},400),n("header").hasClass("active")&&n(".menu-btn").trigger("click")):(n(".lines").removeClass("finish"),n(".lines").removeClass("ready"),n(".lines").addClass("no-lines"),setTimeout(function(){location.href=""+e},2500)),!1}),n(window).on("scroll",function(){n(this).scrollTop()>=n(".section.started").height()?n("body").removeClass("background-enabled"):!n("header").hasClass("active")&&n(".video-bg").length&&n("body").addClass("background-enabled"),100<=n(this).scrollTop()&&1<n(".section").length&&(n(".header").addClass("fixed"),n(".footer").addClass("fixed"),n(".mouse_btn").fadeOut()),n(this).scrollTop()<=100&&1<n(".section").length&&(n(".header").removeClass("fixed"),n(".footer").removeClass("fixed"),n(".mouse_btn").fadeIn())}),n("header").on("click",".menu-btn",function(){return n("header").hasClass("active")?(n("header").removeClass("active"),n(".footer .soc").fadeIn(),n("body").addClass("loaded"),n(".video-bg").length&&n("body").addClass("background-enabled")):(n("header").addClass("active"),n(".footer .soc").hide(),n("body").removeClass("loaded"),n("body").removeClass("background-enabled")),!1}),n(".section.about").on("click touchstart",".btn",function(){location.href=n(this).attr("href")}),n(".section").on("click",".mouse_btn",function(){n("body, html").animate({scrollTop:a-150},800)}),1<n(".section").length&&n(".mouse_btn").show();var t=n(".portfolio-items");t.imagesLoaded(function(){if(t.isotope({percentPosition:!0,itemSelector:".box-item"}),n(".portfolio-items").length){var e=document.getElementsByClassName("wp-post-image");new simpleParallax(e)}}),n(".filters").on("click",".btn-group",function(){var e=n(this).find("input").val();t.isotope({filter:e}),n(".filters .btn-group label").removeClass("glitch-effect"),n(this).find("label").addClass("glitch-effect")}),/\.(?:jpg|jpeg|gif|png)$/i.test(n(".gallery-item:first a").attr("href"))&&n(".gallery-item a").magnificPopup({gallery:{enabled:!0},type:"image",closeBtnInside:!1,mainClass:"mfp-fade"}),n(".has-popup-media").magnificPopup({type:"inline",overflowY:"auto",closeBtnInside:!0,mainClass:"mfp-fade"}),n(".has-popup-image").magnificPopup({type:"image",closeOnContentClick:!0,mainClass:"mfp-fade",image:{verticalFit:!0}}),n(".has-popup-gallery").on("click",function(){var e=n(this).attr("href");return n(e).magnificPopup({delegate:"a",type:"image",closeOnContentClick:!1,mainClass:"mfp-fade",removalDelay:160,fixedContentPos:!1,gallery:{enabled:!0}}).magnificPopup("open"),!1});n(".video-unmuted-bg").length;var s=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),i=!1;/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent)&&(i=!0),n(".video-bg").length&&(n("body").addClass("background-enabled"),i?n(".video-mobile-bg").each(function(){n(this).css("background-image","url("+n(this).data("mobile-preview")+")")}):n(".jarallax-video").length&&n(".jarallax-video").each(function(){var e=n(this).data("volume");s||n(this).jarallax({videoVolume:e}),s&&n(this).jarallax()})),n(".section").length&&n(".top-menu li a").length&&n(window).on("scroll",function(){var t=n(window).scrollTop();n(".top-menu ul li a").each(function(){if(0==n(this).attr("href").indexOf("#section-")){var e=n(this),a=n(e.attr("href"));a.length&&a.offset().top<=t+70&&(n(".top-menu ul li").removeClass("current-menu-item"),e.closest("li").addClass("current-menu-item")),0==t&&n(".top-menu ul li").removeClass("current-menu-item")}})}),n(".single-post-text").each(function(){n(this).find("iframe").wrap('<div class="embed-container"></div>')}),setTimeout(function(){var e=n(".skills.dotted .progress"),a=e.width();e.length&&(e.append('<span class="dg"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></span>'),e.find(".percentage").append('<span class="da"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></span>'),e.find(".percentage .da").css({width:a}))},1e3);var o=n(".skills.circles .progress");o.length&&o.append('<div class="slice"><div class="bar"></div><div class="fill"></div></div>'),n(window).resize(function(){var e=n(window).width(),a=n(window).height();n(".section.started").css({height:a}),e<783&&n(".section.started").css({height:a});var t=n(".skills-list.dotted .progress"),s=t.width();t.length&&t.find(".percentage .da").css({width:s+1})}),n("#cform").validate({rules:{name:{required:!0},message:{required:!0},email:{required:!0,email:!0}},success:"valid",submitHandler:function(){n.ajax({url:"mailer/feedback.php",type:"post",dataType:"json",data:"name="+n("#cform").find('input[name="name"]').val()+"&email="+n("#cform").find('input[name="email"]').val()+"&message="+n("#cform").find('textarea[name="message"]').val(),beforeSend:function(){},complete:function(){},success:function(e){n("#cform").fadeOut(),n(".alert-success").delay(1e3).fadeIn()}})}}),n("#thankyou").length&&initMap()}(jQuery)